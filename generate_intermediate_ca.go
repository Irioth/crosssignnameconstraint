...
func generateIntermediateCA(commonNamePrefix string, permittedDomain string, originalDERBytes []byte, rootDERBytes []byte, rootPrivateKey interface{}) ([]byte, interface{}, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Error parsing root certificate: %s", err)
	}
	...
	derBytes, err := x509.CreateCertificate(rand.Reader, &template, rootCert, publicKey(priv), rootPrivateKey)
	if err != nil {
